...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $q =  "SELECT `id`,`name`, `img`, `price`, `type`, `stock`, `details` FROM `product` WHERE `type`='Vegetable Plants' and `$cat` LIKE '%$search%';";
  ...
  if(mysqli_num_rows($run)>0){
      while($row = $run->fetch_assoc()){
        ...
        $q =  "SELECT `id`,`name`, `img`, `price`, `type`, `stock`, `details` FROM `product` WHERE `type`='Vegetable Plants';";
        $run = mysqli_query($con, $q);
        $html = "";
        if(mysqli_num_rows($run)>0){
            while($row = $run->fetch_assoc()){
...
<body>
    ...
    <div id="cta">
    <h1 class = "cta-heading"><i class="fa-solid fa-carrot"></i> Vegetable Plants</h1>
    </div>
    
    
    <form class="form" action="Vegetable_Plants.php" method="POST">
     ...
     <button onclick="window.location.href='Vegetable_Plants.php'" type="button" class="btn btn-lg btn-block btn-info"><i class="fa-solid fa-list"></i> See All</button>
     </div>
    </form>
