...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $q =  "SELECT `id`,`name`, `img`, `price`, `type`, `stock`, `details` FROM `product` WHERE `type`='Fruit Seeds' and `$cat` LIKE '%$search%';";
  ...
  if(mysqli_num_rows($run)>0){
      while($row = $run->fetch_assoc()){
        ...
        $q =  "SELECT `id`,`name`, `img`, `price`, `type`, `stock`, `details` FROM `product` WHERE `type`='Fruit Seeds';";
        $run = mysqli_query($con, $q);
        $html = "";
        if(mysqli_num_rows($run)>0){
            while($row = $run->fetch_assoc()){
...
<body>
    ...
    <div id="cta">
    <h1 class = "cta-heading"><i class="fa-solid fa-seedling"></i> Fruit Seeds</h1>
    </div>
    
    
    <form class="form" action="Fruit_Seeds.php" method="POST">
     ...
     <button onclick="window.location.href='Fruit_Seeds.php'" type="button" class="btn btn-lg btn-block btn-info"><i class="fa-solid fa-list"></i> See All</button>
     </div>
    </form>
